...
def instructions_keyboard():
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="Требования к оборудованию", callback_data="instruction_hardware")],
...
@dp.callback_query(F.data.startswith("instruction_") & ~F.data.in_({"instruction_tech", "instruction_hardware"}))
async def instruction_placeholder(callback_query: types.CallbackQuery):
    """Placeholder for all instruction buttons, except specific handlers."""
...
@dp.callback_query(F.data == "instruction_hardware")
async def send_hardware_requirements(callback_query: types.CallbackQuery):
    """Sends the hardware requirement PDF files."""
    files_to_send = [
        "documents/hardware_req_1.pdf",
        "documents/hardware_req_2.pdf"
    ]

    try:
        await callback_query.message.answer_chat_action('upload_document')
        await callback_query.message.answer("Отправляю файлы с требованиями к оборудованию...")
        ...
        for file_path in files_to_send:
            try:
                ...
                logger.error(f"Hardware requirement file not found: {file_path}")
                await callback_query.message.answer(f"Ошибка: Файл '{file_path.split('/')[-1]}' не найден.")

        await callback_query.answer() # Acknowledge the initial button press

    except Exception as e:
        logger.error(f"Error sending hardware requirements: {e}")
        await callback_query.answer(
            "Произошла ошибка при отправке файлов.",
            show_alert=True
        )
